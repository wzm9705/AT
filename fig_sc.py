...
data = pd.read_csv('data_with_hk.csv',index_col=())
hk_chg = pd.DataFrame()
lengh = 5 
for g in set(data.code):
...
plt.savefig('fig1 加入深股通后AT变化.png')
plt.show()
'''
at = pd.DataFrame()
for t in list(set(data.date)):
    data_t = data[data.date == t]
    at.loc[t,'MBM_vol'] = np.mean(data_t[data_t.BK == 'ZB'].at_volume)

    at.loc[t,'GEM_vol'] = np.mean(data_t[data_t.BK == 'CYB'].at_volume)
    at.loc[t,'SME_vol'] = np.mean(data_t[data_t.BK == 'ZXB'].at_volume)
    at.loc[t,'MBM_tra'] = np.mean(data_t[data_t.BK == 'ZB'].at_trades)
    at.loc[t,'GEM_tra'] = np.mean(data_t[data_t.BK == 'CYB'].at_trades)
    at.loc[t,'SME_tra'] = np.mean(data_t[data_t.BK == 'ZXB'].at_trades)
    at.loc[t,'MBM_otr'] = np.mean(data_t[data_t.BK == 'ZB'].iv_tor)
    at.loc[t,'GEM_otr'] = np.mean(data_t[data_t.BK == 'CYB'].iv_tor)
    at.loc[t,'SME_otr'] = np.mean(data_t[data_t.BK == 'ZXB'].iv_tor)   
